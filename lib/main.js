...
export default class SinkMemory extends Sink {
	...
	_state = new Map();
	/** Generation counter per path — incremented on every successful write. */
	...
	write(filePath, contentType, options = {}) {
		return new Promise((resolve, reject) => {
			...
			if (options.ifNotExists && this._state.has(pathname)) {
			...
			if (options.ifGenerationMatch !== undefined) {
				const current = String(this._generations.get(pathname) ?? 0);
				if (String(options.ifGenerationMatch) !== current) {
					const err = new Error(
						`Generation mismatch for ${filePath}: expected ${options.ifGenerationMatch}, got ${current}`,
					);
					// @ts-ignore
					err.code = "CONFLICT";
					this._counter.inc({ labels: { operation } });
			...
			stream.on("finish", () => {
				...
				this._generations.set(
					pathname,
					(this._generations.get(pathname) ?? 0) + 1,
				);
	...
	read(filePath) {
		return new Promise((resolve, reject) => {
			...
			// Expose the generation for compare-and-swap writes.
			// @ts-ignore — ReadFile.generation is available in @eik/common >= next
			// release; direct property assignment works with all ReadFile versions.
			obj.generation = String(this._generations.get(pathname) ?? 0);
